...
The reason for maintaining the branches separated is to keep the code for the actual project evaluation as simple as possible (the project does not actually require the implementation of logging/tests) and limit the number of dependencies. Among the other things, the code in `log_and_test` hasn't really been tested on multiple systems, and so at the present stage there is no guarantee on its portability).
